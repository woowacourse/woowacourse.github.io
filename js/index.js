const INDEX_APP = (() => {
  ...
  const createBannerBgImages = () => {
    const indexArray = [1, 2, 3, 4, 5, 6, 7, 8]
    const sortedIndex = indexArray.sort(() => Math.random() - Math.random())
    sortedIndex.map((index) => document.getElementById('banner-cover-container').insertAdjacentHTML('afterbegin', bannerBgTemplate(index)))
  }
